//2021.10.28 depoly
//存储绑定关系的合约，预留了批量设置的接口 以便于迁移之前的绑定关系，初始部署时的owner为部署地址，迁移完绑定关系后将交给多签地址
...
contract relationship is Ownable{
    ...
    constructor(address _defultFather, address _leoToken, address _SGRTToken){
    ...
    function batchSetRealtionship(address[] memory _sons, address[] memory _fathers, address[] memory _grandFathers) public onlyOwner(){
        ...
        for (uint256 i = 0; i < _length ; i++){
            father[_sons[i]] = _fathers[i];
            grandFather[_sons[i]] = _grandFathers[i];
        }
    }
